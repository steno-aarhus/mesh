...
raw_data_location <- "/run/user/1000/gvfs/smb-share:server=uni.au.dk,share=users/au597503/Documents/ukbiobank/ukb_data_cleaned.csv"

# tictoc::tic()
# variable_names <- vroom::vroom_lines(raw_data_location, n_max = 1)
# tictoc::toc()
...
calc_leg_measures <- function(.tbl) {
    .tbl %>%
        ...
        filter(leg_length_0_0 < 120, leg_length_0_0 > 60)
}

calc_t2dm_status <- function(.tbl) {
    .tbl %>%
        # Convert illness variables to either with or without type 2 diabetes
        mutate_at(vars(matches("^noncancer_illness_")),
                  ~ if_else(. == 1223, TRUE, FALSE)) %>%
        # Calculate t2dm status by summing non-cancer illnesses
        mutate(t2dm_status = rowSums(select(., matches(
            "^noncancer_illness_"
        )), na.rm = TRUE)) %>%
        # Keep only age of diagnosis for participant that have t2dm
        mutate_at(vars(matches("^interpolated_age_of_")),
                  ~ if_else(t2dm_status >= 1, as.numeric(.), NA_real_))
}

# tictoc::tic()
ukb_data_smaller <- ukb_data %>%
    # Filter out type 1 diabetes
    filter_at(vars(matches("^noncancer_illness_")), all_vars(. != 1222 | is.na(.))) %>%
    # Drop missing sitting heights
    filter(!is.na(sitting_height_f20015_0_0)) %>%
    # Drop too big or too small values
    filter(
        standing_height_f50_0_0 > 150,
...
ukb_data_smallest <- ukb_data_smaller %>%
    ...
    select(eid, ends_with("0_0"), matches("^(diastolic|systolic)_.*0_[0-1]$"),
           matches("^(interpolated_age|noncancer_illness)_.*_0_[0-9].*$"))
# tictoc::toc()
